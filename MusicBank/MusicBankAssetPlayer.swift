...
//    unowned let nowPlayableBehavior: NowPlayable
    ...
    init(assets: [AssetItemModel]) throws {
        ...
        self.playerItems = playableAssets.map {
            AVPlayerItem(asset: AVURLAsset(url: $0.assetURL!), automaticallyLoadedAssetKeys: [MusicBankAssetPlayer.mediaSelectionKey])
        }
...
//                      NSLog(@"视频的加载进度是:%%%f", durationSeconds / self.total * 100);
    ...
    private func handlePlaybackChange() {
        ...
        if asset.statusOfValue(forKey: MusicBankAssetPlayer.mediaSelectionKey, error: nil) == .loaded {
